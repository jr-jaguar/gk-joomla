 ...
 */
class StudentModelLists extends JModelList {
    ...
    protected function getListQuery() {
        ...
        $query->from('`#__student_list` AS a');
    ...
    $query->join('LEFT', '#__users AS uc ON uc.id=a.checked_out');
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.fio LIKE '.$search.'  OR  a.grup LIKE '.$search.' )');
            }
        }
		...
		if ($filter_birthday_from) {
			$query->where("a.birthday >= '".$filter_birthday_from."'");
		}
		$filter_birthday_to = $this->state->get("filter.birthday.to");
		if ($filter_birthday_to) {
			$query->where("a.birthday <= '".$filter_birthday_to."'");
		}
